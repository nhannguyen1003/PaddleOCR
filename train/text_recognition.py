...
def print_draw_crop_rec_res( img_crop_list, img_name):
        ...
        for bno in range(bbox_num):
          ...
          crop_name_w = "./vietnamese/train/img_crop/{}".format(crop_name)
          cv2.imwrite(crop_name_w, img_crop_list[bno])
          crop_label.write("{0}\t{1}\n".format(crop_name, text[bno]))
 
if not os.path.exists('./vietnamese/train/'):
  os.mkdir('./vietnamese/train/') 
 
crop_label = open('./vietnamese/train/crop_label.txt','w', encoding='utf8')
with open('./train_label.txt','r', encoding='utf8') as file_text:
...
for img_file in img_files:
  ...
  image_file = './vietnamese/train_images/' +img_file.split('\t')[0]
  ...
  if not os.path.exists('./vietnamese/train/img_crop'):
    os.mkdir('./vietnamese/train/img_crop') 
